...
internal static class TimeSpanFormat
{
    ...
    internal static readonly FormatLiterals PositiveInvariantFormatLiterals =
        FormatLiterals.InitInvariant(isNegative: false);

    internal static readonly FormatLiterals NegativeInvariantFormatLiterals =
        FormatLiterals.InitInvariant(isNegative: true);
    ...
    internal static string Format(TimeSpan value, string format, IFormatProvider formatProvider)
    {
        ...
        if (format.Length == 1)
        {
            ...
            if ((c | 0x20) == 103)
            {
                return FormatG(value, DateTimeFormatInfo.GetInstance(formatProvider),
                    (c == 'G') ? StandardFormat.G : StandardFormat.g);
            }

            throw new FormatException("SR.Format_InvalidString");
        }

        return StringBuilderCache.GetStringAndRelease(FormatCustomized(value, format,
        ...
        Span<char> destination, out int charsWritten)
    {
        ...
        num3 = Math.DivRem((ulong)num2, 10000000uL, out var result);
        valueWithoutTrailingZeros = (uint)result;
        goto IL_0045;
        IL_0045:
        int num4 = 0;
        switch (format)
        {
            ...
            default:
                if (valueWithoutTrailingZeros != 0)
                {
                    num4 = 7 - FormattingHelpers.CountDecimalTrailingZeros(valueWithoutTrailingZeros,
                        out valueWithoutTrailingZeros);
                    num += num4 + 1;
                }
        ...
        if (num3 != 0)
        {
            num5 = Math.DivRem(num3, 60uL, out result2);
        }
        ...
        if (num5 != 0)
        {
            num6 = Math.DivRem(num5, 60uL, out result3);
        }
        ...
        if (num6 != 0)
        {
            num7 = Math.DivRem((uint)num6, 24u, out result4);
        }
        ...
        destination[num10++] = ':';
        WriteTwoDigits((uint)result3, destination.Slice(num10));
        num10 += 2;
        destination[num10++] = ':';
        WriteTwoDigits((uint)result2, destination.Slice(num10));
        num10 += 2;
        if (num4 != 0)
        {
    ...
    {
        return FormatCustomized(value, format.AsSpan(), dtfi, result);
    }

    private static StringBuilder FormatCustomized(TimeSpan value, ReadOnlySpan<char> format, DateTimeFormatInfo dtfi,
        StringBuilder result = null)
    {
        bool flag = false;
        if (result == null)
            ...
            switch (c)
            {
                ...
                case 'f':
                    ...
                    if (num6 <= 7)
                    {
                        ...
                        result.AppendSpanFormattable(num4, DateTimeFormat.fixedNumberFormats[num6 - 1],
                ...
                case 'F':
                    ...
                    if (num6 <= 7)
                    {
                        ...
                        if (num7 > 0)
                        {
                            result.AppendSpanFormattable(num4, DateTimeFormat.fixedNumberFormats[num7 - 1],
                                CultureInfo.InvariantCulture);
                        }
                ...
                case '%':
                {
                    ...
                    if (num5 >= 0 && num5 != 37)
                    {
                        ...
                        StringBuilder stringBuilder = FormatCustomized(value,
                            MemoryMarshal.CreateReadOnlySpan(ref reference, 1), dtfi, result);
                        num6 = 2;
                        continue;
                    }
