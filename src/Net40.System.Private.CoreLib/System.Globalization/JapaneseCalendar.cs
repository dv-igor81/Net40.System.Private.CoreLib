...
public class JapaneseCalendar : Calendar
{
    ...
    internal static EraInfo[] GetEraInfo()
    {
        ...
        if (obj == null)
        {
            obj = (s_japaneseEraInfo = GetJapaneseEras());
            if (obj == null)
            {
                obj = new EraInfo[5]
                {
    ...
    private static EraInfo[] GetJapaneseEras()
    {
        ...
        try
        {
            using RegistryKey registryKey =
                Registry.LocalMachine.OpenSubKey("System\\CurrentControlSet\\Control\\Nls\\Calendars\\Japanese\\Eras");
            if (registryKey == null)
            {
                return null;
            }
            ...
            if (valueNames != null && valueNames.Length != 0)
            {
                ...
                {
                    EraInfo eraFromValue =
                        GetEraFromValue(valueNames[i], registryKey.GetValue(valueNames[i])?.ToString());
    ...
    private static EraInfo GetEraFromValue(string value, string data)
    {
        ...
        if (!int.TryParse(readOnlySpan.Slice(0, 4).ToString(), NumberStyles.None, NumberFormatInfo.InvariantInfo,
                out var result) ||
            !int.TryParse(readOnlySpan.Slice(5, 2).ToString(), NumberStyles.None, NumberFormatInfo.InvariantInfo,
                out var result2) || !int.TryParse(readOnlySpan.Slice(8, 2).ToString(), NumberStyles.None,
                NumberFormatInfo.InvariantInfo, out var result3))
        {
            return null;
        }
