...
internal class GregorianCalendarHelper
{
    ...
    private int GetYearOffset(int year, int era, bool throwOnError)
    {
        ...
        for (int i = 0; i < m_EraInfo.Length; i++)
        {
            ...
            throw new ArgumentOutOfRangeException("year",
                SR.Format("SR.ArgumentOutOfRange_Range", m_EraInfo[i].minEraYear, m_EraInfo[i].maxEraYear));
        }
    ...
    public DateTime AddMonths(DateTime time, int months)
    {
        ...
        int[] array = ((datePart % 4 == 0 && (datePart % 100 != 0 || datePart % 400 == 0))
            ? DaysToMonth366
            : DaysToMonth365);
    ...
    public bool IsLeapDay(int year, int month, int day, int era)
    {
        if (day < 1 || day > GetDaysInMonth(year, month, era))
        {
            throw new ArgumentOutOfRangeException("day",
                SR.Format("SR.ArgumentOutOfRange_Range", 1, GetDaysInMonth(year, month, era)));
        }
