...
public class BinaryPrimitivesEx
{
	...
	{
		return (!BitConverter.IsLittleEndian) ? BitConverterEx.Int32BitsToSingle(BinaryPrimitives.ReverseEndianness(MemoryMarshal.Read<int>(source))) : MemoryMarshal.Read<float>(source);
	}
	...
	public static bool TryReadSingleLittleEndian(ReadOnlySpan<byte> source, out float value)
	{
		if (!BitConverter.IsLittleEndian)
		{
			...
			value = BitConverterEx.Int32BitsToSingle(BinaryPrimitives.ReverseEndianness(tmp));
			return success;
		}
	...
	public static void WriteSingleLittleEndian(Span<byte> destination, float value)
	{
		if (!BitConverter.IsLittleEndian)
		{
			int tmp = BinaryPrimitives.ReverseEndianness(BitConverterEx.SingleToInt32Bits(value));
