...
internal static class IriHelper
{
	...
	internal static bool CheckIriUnicodeRange(char highSurr, char lowSurr, ref bool surrogatePair, bool isQuery)
	{
		...
		{
			surrogatePair = true;
			Span<char> span = stackalloc char[2] { highSurr, lowSurr };
			ReadOnlySpan<char> value = span;
			string strA = new string(value.ToArray());
		...
		for (; i < end; i++)
		{
			...
			if ((c = pInput[i]) == '%')
			{
				if (i + 2 < end)
				{
					...
					i--;
					Encoding encoding = Encoding.GetEncoding(Encoding.UTF8.CodePage, new EncoderReplacementFallback(""), new DecoderReplacementFallback(""));
			...
			else if (c > '\u007f')
			{
				...
				{
					if (!UriHelper.IsBidiControlCharacter(c) || !UriParser.DontKeepUnicodeBidiFormattingCharacters)
					{
						ptr[destOffset++] = pInput[i];
					}
				}
			...
			if (num < 12)
			{
				...
				fixed (char* destination = array4)
				{
					BufferEx.MemoryCopy(ptr, destination, num3 * 2, destOffset * 2);
				}
