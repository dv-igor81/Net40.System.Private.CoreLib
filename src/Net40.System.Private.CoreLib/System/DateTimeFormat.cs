...
using HebrewNumber = System.Globalization.Net40.HebrewNumber;
	...
	private static StringBuilder FormatCustomized(DateTime dateTime, ReadOnlySpan<char> format, DateTimeFormatInfo dtfi, TimeSpan offset, StringBuilder result)
	{
		...
		for (int i = 0; i < format.Length; i += num)
		{
			...
			case 'y':
			{
				...
				num = ParseRepeatPattern(format, i, c);
				if (flag3 && !LocalAppContextSwitches.FormatJapaneseFirstYearAsANumber && year == 1 && ((i + num < format.Length && format[i + num] == "年"[0]) || (i + num < format.Length - 1 && format[i + num] == '\'' && format[i + num + 1] == "年"[0])))
				{
					result.Append("元"[0]);
				}
				...
				{
					result.Append(year.ToString("D" + num, CultureInfo.InvariantCulture));
				}
				timeOnly = false;
				break;
			}
