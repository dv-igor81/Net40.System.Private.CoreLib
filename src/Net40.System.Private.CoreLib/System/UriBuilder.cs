...
public class UriBuilder
{
    ...
    private string _schemeDelimiter = System.Uri.SchemeDelimiter;
    ...
    private string? Extra
    {
        set
        {
            ...
            if (value.Length > 0)
            {
                ...
                if (value[0] != '?')
                {
                    throw new ArgumentException("SR.Argument_ExtraNotValid", "value");
                }
    ...
    public string Scheme
    {
        ...
        set
        {
            ...
            if (value.Length != 0)
            {
                ...
                {
                    throw new ArgumentException("SR.net_uri_BadScheme", "value");
                }
    ...
    private void Init(Uri uri)
    {
        ...
        _port = uri.Port;
        _scheme = uri.Scheme;
        _schemeDelimiter = (uri.HasAuthority ? System.Uri.SchemeDelimiter : ":");
        ...
        : this(scheme, host, port, path)
    {
        ...
        catch (Exception ex)
        {
            ...
            throw new ArgumentException("SR.Argument_ExtraNotValid", "extraValue");
        }
    }
    ...
    public override string ToString()
    {
        ...
        if (_scheme.Length != 0)
        {
            ...
            {
                _schemeDelimiter =
                    ((syntax.InFact(UriSyntaxFlags.MustHaveAuthority) || (_host.Length != 0 &&
            ...
            {
                _schemeDelimiter = ((_host.Length != 0) ? System.Uri.SchemeDelimiter : ":");
            }
        }

        string text = ((_scheme.Length != 0) ? (_scheme + _schemeDelimiter) : string.Empty);
        return text + _username + ((_password.Length > 0) ? (":" + _password) : string.Empty) +
               ((_username.Length > 0) ? "@" : string.Empty) + _host +
               ((_port != -1 && _host.Length > 0) ? (":" + _port) : string.Empty) +
               ((_host.Length > 0 && _path.Length != 0 && _path[0] != '/') ? "/" : string.Empty) + _path + _query +
               _fragment;
    }
}