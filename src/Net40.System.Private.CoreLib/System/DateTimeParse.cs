...
using System.Globalization.Net40;
using System.Runtime.CompilerServices;
using System.Text;
using Calendar = System.Globalization.Net40.Calendar;
using DateTimeFormatInfo = System.Globalization.Net40.DateTimeFormatInfo;
using GregorianCalendar = System.Globalization.Net40.GregorianCalendar;
using JapaneseCalendar = System.Globalization.Net40.JapaneseCalendar;
...
internal static class DateTimeParse
{
    ...
    internal static DateTime ParseExact(ReadOnlySpan<char> s, ReadOnlySpan<char> format, DateTimeFormatInfo dtfi,
    ...
    internal static DateTime ParseExact(ReadOnlySpan<char> s, ReadOnlySpan<char> format, DateTimeFormatInfo dtfi,
        DateTimeStyles style, out TimeSpan offset)
    {
        DateTimeResult result = default(DateTimeResult);
    ...
    internal static bool TryParseExact(ReadOnlySpan<char> s, ReadOnlySpan<char> format, DateTimeFormatInfo dtfi,
    ...
    internal static bool TryParseExact(ReadOnlySpan<char> s, ReadOnlySpan<char> format, DateTimeFormatInfo dtfi,
        DateTimeStyles style, out DateTime result, out TimeSpan offset)
    {
        result = DateTime.MinValue;
        offset = TimeSpan.Zero;
        DateTimeResult result2 = default(DateTimeResult);
    ...
    internal static bool TryParseExact(ReadOnlySpan<char> s, ReadOnlySpan<char> format, DateTimeFormatInfo dtfi,
        DateTimeStyles style, ref DateTimeResult result)
    {
        if (s.Length == 0)
        {
    ...
    internal static DateTime ParseExactMultiple(ReadOnlySpan<char> s, string[] formats, DateTimeFormatInfo dtfi,
        DateTimeStyles style)
    {
        DateTimeResult result = default(DateTimeResult);
        result.Init(s);
    ...
    internal static bool TryParseExactMultiple(ReadOnlySpan<char> s, string[] formats, DateTimeFormatInfo dtfi,
        DateTimeStyles style, out DateTime result, out TimeSpan offset)
    {
        result = DateTime.MinValue;
        offset = TimeSpan.Zero;
    ...
    private static bool Lex(DS dps, ref __DTString str, ref DateTimeToken dtok, ref DateTimeRawInfo raw,
        ref DateTimeResult result, ref DateTimeFormatInfo dtfi, DateTimeStyles styles)
    {
        dtok.dtt = DTT.Unk;
        ...
        switch (tokenType)
        {
            ...
            case TokenType.YearNumberToken:
            {
                ...
                if ((dps == DS.T_NNt || dps == DS.T_Nt) && str.Index < str.Length - 1 &&
                    !HandleTimeZone(ref str, ref result))
                {
                    return false;
                }
                    ...
                    if (raw.year == -1)
                    {
                                    ...
                                    str.GetSeparatorToken(dtfi, out indexBeforeSeparator, out charBeforeSeparator))
                        {
                            ...
                            case TokenType.SEP_DateOrOffset:
                                if (dateParsingStates[(int)dps][13] == DS.ERROR &&
                                    dateParsingStates[(int)dps][12] > DS.ERROR)
                                {
                ...
                switch (separatorToken = str.GetSeparatorToken(dtfi, out indexBeforeSeparator, out charBeforeSeparator))
                {
                    ...
                    case TokenType.SEP_Pm:
                        if (raw.timeMark == TM.NotSet)
                        {
                            ...
                            if (dps == DS.D_NN && !ProcessTerminalState(DS.DX_NN, ref str, ref result, ref styles,
                                    ref raw, dtfi))
                            {
                                return false;
                            }
                    ...
                    case TokenType.SEP_DateOrOffset:
                        ...
                        break;
                    case TokenType.SEP_Time:
                        if (raw.hasSameDateAndTimeSeparators && (dps == DS.D_Y || dps == DS.D_YN || dps == DS.D_YNd ||
                                                                 dps == DS.D_YM || dps == DS.D_YMd))
                        {
                            dtok.dtt = DTT.NumDatesep;
                            raw.AddNumber(dtok.num);
                        }
            ...
            case TokenType.HebrewNumber:
            {
                ...
                if (tokenValue >= 100)
                {
                    ...
                    {
                        raw.year = tokenValue;
                        TokenType tokenType2 = (separatorToken =
                            str.GetSeparatorToken(dtfi, out indexBeforeSeparator, out charBeforeSeparator));
                        if (tokenType2 != TokenType.SEP_End)
                        {
                            if (tokenType2 != TokenType.SEP_Space)
                            {
                                if (tokenType2 != TokenType.SEP_DateOrOffset ||
                                    dateParsingStates[(int)dps][12] <= DS.ERROR)
                                {
                                    result.SetBadDateTimeFailure();
                                    return false;
            ...
            case TokenType.UnknownToken:
                ...
                {
                    result.SetFailure(ParseFailureKind.FormatWithOriginalDateTimeAndParameter,
                        "Format_UnknownDateTimeWord", str.Index);
                    return false;
                }
    ...
    private static bool GetDayOfNN(ref DateTimeResult result, ref DateTimeStyles styles, ref DateTimeRawInfo raw,
    ...
    private static bool GetDayOfNNN(ref DateTimeResult result, ref DateTimeRawInfo raw, DateTimeFormatInfo dtfi)
    {
        ...
        {
            case 0:
                if (TryAdjustYear(ref result, number, out adjustedYear) &&
                ...
                break;
            case 1:
                if (TryAdjustYear(ref result, number3, out adjustedYear) &&
    ...
    private static bool GetDayOfMN(ref DateTimeResult result, ref DateTimeStyles styles, ref DateTimeRawInfo raw,
        DateTimeFormatInfo dtfi)
    {
        if ((result.flags & ParseFlags.HaveDate) != 0)
        {
        ...
        if (order == 7)
        {
            ...
            if (order2 == 5)
            {
                if (!TryAdjustYear(ref result, raw.GetNumber(0), out var adjustedYear) ||
    ...
    private static bool GetHebrewDayOfNM(ref DateTimeResult result, ref DateTimeRawInfo raw, DateTimeFormatInfo dtfi)
    {
        ...
        if ((order == 7 || order == 6) &&
            result.calendar.IsValidDay(result.Year, result.Month, raw.GetNumber(0), result.era))
        {
    ...
    private static bool GetDayOfNM(ref DateTimeResult result, ref DateTimeStyles styles, ref DateTimeRawInfo raw,
        DateTimeFormatInfo dtfi)
    {
        if ((result.flags & ParseFlags.HaveDate) != 0)
        {
    ...
    private static bool GetDayOfMNN(ref DateTimeResult result, ref DateTimeRawInfo raw, DateTimeFormatInfo dtfi)
    {
        ...
        {
            case 1:
                if (TryAdjustYear(ref result, number2, out adjustedYear) &&
                    result.calendar.IsValidDay(adjustedYear, raw.month, number, result.era))
                {
                ...
                if (TryAdjustYear(ref result, number, out adjustedYear) &&
                    result.calendar.IsValidDay(adjustedYear, raw.month, number2, result.era))
                {
    ...
    private static bool GetDateOfNNDS(ref DateTimeResult result, ref DateTimeRawInfo raw, DateTimeFormatInfo dtfi)
    {
        ...
        else if ((result.flags & ParseFlags.HaveMonth) != 0 && (result.flags & ParseFlags.HaveYear) == 0 &&
                 (result.flags & ParseFlags.HaveDay) == 0)
        {
            ...
            if (order == 0)
            {
                if (TryAdjustYear(ref result, raw.GetNumber(0), out adjustedYear) &&
                    SetDateYMD(ref result, adjustedYear, result.Month, raw.GetNumber(1)))
                {
                    return true;
                }
            }
            else if (TryAdjustYear(ref result, raw.GetNumber(1), out adjustedYear) &&
    ...
    private static bool ProcessDateTimeSuffix(ref DateTimeResult result, ref DateTimeRawInfo raw,
    ...
    internal static bool ProcessHebrewTerminalState(DS dps, ref __DTString str, ref DateTimeResult result,
        ref DateTimeStyles styles, ref DateTimeRawInfo raw, DateTimeFormatInfo dtfi)
    {
        switch (dps)
        {
            case DS.DX_MNN:
    ...
    internal static bool ProcessTerminalState(DS dps, ref __DTString str, ref DateTimeResult result,
        ref DateTimeStyles styles, ref DateTimeRawInfo raw, DateTimeFormatInfo dtfi)
    {
        bool flag = true;
        switch (dps)
    ...
    internal static bool TryParse(ReadOnlySpan<char> s, DateTimeFormatInfo dtfi, DateTimeStyles styles,
        out DateTime result)
    {
        result = DateTime.MinValue;
        DateTimeResult result2 = default(DateTimeResult);
    ...
    internal static bool TryParse(ReadOnlySpan<char> s, DateTimeFormatInfo dtfi, DateTimeStyles styles,
        out DateTime result, out TimeSpan offset)
    {
        result = DateTime.MinValue;
        offset = TimeSpan.Zero;
        DateTimeResult result2 = default(DateTimeResult);
    ...
    internal unsafe static bool TryParse(ReadOnlySpan<char> s, DateTimeFormatInfo dtfi, DateTimeStyles styles,
        ref DateTimeResult result)
    {
        ...
        do
        {
            ...
            {
                if (!ProcessHebrewTerminalState(dS, ref str, ref result, ref styles, ref raw, dtfi))
                {
                    return false;
                }
            }
        ...
        if (!result.calendar.TryToDateTime(result.Year, result.Month, result.Day, result.Hour, result.Minute,
                result.Second, 0, result.era, out var result2))
        {
            result.SetFailure(ParseFailureKind.FormatBadDateTimeCalendar, "Format_BadDateTimeCalendar");
            return false;
        }
    ...
    private static bool AdjustTimeZoneToLocal(ref DateTimeResult result, bool bTimeOnly)
    {
        ...
        {
            ticks -= result.timeZoneOffset.Ticks;
            ticks += local.GetUtcOffset(bTimeOnly ? DateTime.Now : result.parsedDate,
        ...
        else
        {
            ...
            else
            {
                ...
                ticks += TimeZoneInfo
                    .GetUtcOffsetFromUtc(time, TimeZoneInfo.Local, out isDaylightSavings, out isAmbiguousLocalDst)
                    .Ticks;
            }
        }
    ...
    private static bool ParseISO8601(ref DateTimeRawInfo raw, ref __DTString str, DateTimeStyles styles,
        ref DateTimeResult result)
    {
        ...
        if (!defaultInstance.TryToDateTime(raw.year, raw.GetNumber(0), raw.GetNumber(1), result4, result5, result2, 0,
                result.era, out var result6))
        {
            result.SetFailure(ParseFailureKind.FormatBadDateTimeCalendar, "Format_BadDateTimeCalendar");
            return false;
        }
    ...
    {
        if ((result.flags & ParseFlags.CaptureOffset) != 0 && (result.Month != -1 || result.Day != -1) &&
    ...
    private static string ExpandPredefinedFormat(ReadOnlySpan<char> format, ref DateTimeFormatInfo dtfi,
        ref ParsingInfo parseInfo, ref DateTimeResult result)
    {
        switch (format[0])
        {
            case 'O':
    ...
    {
        if (LocalAppContextSwitches.EnforceLegacyJapaneseDateParsing || dtfi.Calendar.ID != CalendarId.JAPAN ||
            !str.GetNext())
        {
            return false;
        }
    ...
    private static void ConfigureFormatR(ref DateTimeFormatInfo dtfi, ref ParsingInfo parseInfo,
        ref DateTimeResult result)
    {
        parseInfo.calendar = GregorianCalendar.GetDefaultInstance();
        dtfi = DateTimeFormatInfo.InvariantInfo;
    ...
    private static bool ParseByFormat(ref __DTString str, ref __DTString format, ref ParsingInfo parseInfo,
        DateTimeFormatInfo dtfi, ref DateTimeResult result)
    {
        int returnValue = 0;
        int result2 = 0;
        int result3 = 0;
        ...
        switch (@char)
        {
            ...
            case 'M':
                ...
                if (returnValue <= 2)
                {
                    if (!ParseDigits(ref str, returnValue, out result3) && (!parseInfo.fCustomNumberParser ||
                                                                            !parseInfo.parseNumberDelegate(ref str,
                                                                                returnValue, out result3)))
                    {
                        result.SetBadDateTimeFailure();
                        return false;
                    }
            ...
            case 'd':
                ...
                if (returnValue <= 2)
                {
                    if (!ParseDigits(ref str, returnValue, out result4) && (!parseInfo.fCustomNumberParser ||
                                                                            !parseInfo.parseNumberDelegate(ref str,
                                                                                returnValue, out result4)))
                    {
                        result.SetBadDateTimeFailure();
                        return false;
                    }
            ...
            default:
                if (@char == ' ')
                {
                    ...
                    {
                        if (parseInfo.fAllowTrailingWhite && format.GetNext() &&
                            ParseByFormat(ref str, ref format, ref parseInfo, dtfi, ref result))
                        {
                            return true;
                        }
    ...
    internal static bool TryParseQuoteString(ReadOnlySpan<char> format, int pos, StringBuilder result,
        out int returnValue)
    {
        returnValue = 0;
    ...
    private static bool DoStrictParse(ReadOnlySpan<char> s, ReadOnlySpan<char> formatParam, DateTimeStyles styles,
        DateTimeFormatInfo dtfi, ref DateTimeResult result)
    {
        ...
        else if ((parseInfo.timeMark == TM.AM && result.Hour >= 12) ||
        ...
        if (!flag && dtfi.HasYearMonthAdjustment && !dtfi.YearMonthAdjustment(ref result.Year, ref result.Month,
        ...
        if (!parseInfo.calendar.TryToDateTime(result.Year, result.Month, result.Day, result.Hour, result.Minute,
                result.Second, 0, result.era, out result.parsedDate))
        {
            result.SetFailure(ParseFailureKind.FormatBadDateTimeCalendar, "Format_BadDateTimeCalendar");
            return false;
        }
        ...
        if (result.fraction > 0.0 &&
            !result.parsedDate.TryAddTicks((long)Math.Round(result.fraction * 10000000.0), out result.parsedDate))
        {
            result.SetBadDateTimeFailure();
            return false;
        }
    ...
    {
        if ((uint)source.Length < 27u || source[4] != '-' || source[7] != '-' || source[10] != 'T' ||
        ...
        double num22 = (double)(num15 * 1000000 + num16 * 100000 + num17 * 10000 + num18 * 1000 + num19 * 100 +
                                num20 * 10 + num21) / 10000000.0;
    ...
    private static Exception GetDateTimeParseException(ref DateTimeResult result)
    {
        ...
        {
            ParseFailureKind.ArgumentNull => new ArgumentNullException(result.failureArgumentName,
            ...
            ParseFailureKind.FormatWithParameter => new FormatException(
                SR.Format(SR.GetResourceString(result.failureMessageID), result.failureMessageFormatArgument)),
            ParseFailureKind.FormatBadDateTimeCalendar => new FormatException(SR.Format(
                SR.GetResourceString(result.failureMessageID), new string(result.originalDateTimeString),
                result.calendar)),
            ParseFailureKind.FormatWithOriginalDateTime => new FormatException(
                SR.Format(SR.GetResourceString(result.failureMessageID), new string(result.originalDateTimeString))),
            ParseFailureKind.FormatWithFormatSpecifier => new FormatException(
                SR.Format(SR.GetResourceString(result.failureMessageID), new string(result.failedFormatSpecifier))),
            ParseFailureKind.FormatWithOriginalDateTimeAndParameter => new FormatException(
                SR.Format(SR.GetResourceString(result.failureMessageID), new string(result.originalDateTimeString),
