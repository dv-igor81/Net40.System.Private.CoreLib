...
public sealed class ThreadPoolBoundHandle : IDisposable
{
    ...
    [CLSCompliant(false)]
    public unsafe NativeOverlapped* AllocateNativeOverlapped(IOCompletionCallback callback, object? state,
        object? pinData)
    {
        if (callback == null)
        {
        ...
        ThreadPoolBoundHandleOverlapped threadPoolBoundHandleOverlapped =
            new ThreadPoolBoundHandleOverlapped(callback, state, pinData, null);
    ...
    private static ThreadPoolBoundHandle BindHandleCore(SafeHandle handle)
    {
        ...
        {
            ExceptionEx exEx = (ExceptionEx)ex;
            if (exEx.HResult == -2147024890)
            {
                throw new ArgumentException("SR.Argument_InvalidHandle", "handle");
            }

            if (exEx.HResult == -2147024809)
            {
