...
internal sealed class IOControlKeepAlive
{
    ...
    private static readonly ConditionalWeakTable<SafeSocketHandle, IOControlKeepAlive> s_socketKeepAliveTable =
        new ConditionalWeakTable<SafeSocketHandle, IOControlKeepAlive>();

    [ThreadStatic] private static byte[] s_keepAliveValuesBuffer;
    ...
    public static SocketError Get(SafeSocketHandle handle, SocketOptionName optionName, byte[] optionValueSeconds,
        ref int optionLength)
    {
        if (optionValueSeconds == null ||
            !BitConverterEx.TryWriteBytes(optionValueSeconds.AsSpan(), Get(handle, optionName)))
        {
    ...
    {
        IOControlKeepAlive value =
            s_socketKeepAliveTable.GetValue(handle, handle => new IOControlKeepAlive());
        if (optionName == SocketOptionName.TypeOfService)
        {
    ...
    private static bool SupportsKeepAliveViaSocketOption()
    {
        AddressFamily addressFamily =
            (Socket.OSSupportsIPv4 ? AddressFamily.InterNetwork : AddressFamily.InterNetworkV6);
        ...
        SocketError socketError = Interop.Winsock.setsockopt(socket.SafeHandle, SocketOptionLevel.Tcp,
            SocketOptionName.TypeOfService, ref optionValue, 4);
        int optionValue2 = MillisecondsToSeconds(1000u);
        SocketError socketError2 = Interop.Winsock.setsockopt(socket.SafeHandle, SocketOptionLevel.Tcp,
            SocketOptionName.BlockSource, ref optionValue2, 4);
    ...
    {
        bool flag = BitConverterEx.TryWriteBytes(buffer.AsSpan(), 1u) &
                    BitConverterEx.TryWriteBytes(buffer.AsSpan(4), _timeMs) &
                    BitConverterEx.TryWriteBytes(buffer.AsSpan(8), _intervalMs);
    }
}