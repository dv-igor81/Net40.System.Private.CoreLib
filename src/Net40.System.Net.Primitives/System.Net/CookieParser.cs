...
internal class CookieParser
{
	...
	private static FieldInfo IsQuotedDomainField
	{
		get
		{
				if (s_isQuotedDomainField == null)
				{
					...
					FieldInfo field = typeof(Net40.Cookie).GetField("IsQuotedDomain", bindingFlags);
	...
	private static FieldInfo IsQuotedVersionField
	{
		get
		{
				if (s_isQuotedVersionField == null)
				{
					BindingFlags bindingFlags = BindingFlags.Instance;
					bindingFlags |= BindingFlags.NonPublic;
					FieldInfo field = typeof(Net40.Cookie).GetField("IsQuotedVersion", bindingFlags);
	...
	private static bool InternalSetNameMethod(Net40.Cookie cookie, string value)
	{
			return cookie.InternalSetName(value);
		}

	internal Net40.Cookie Get()
	{
		Net40.Cookie cookie = null;
			bool flag = false;
			bool flag2 = false;
			bool flag3 = false;
			bool flag4 = false;
			...
			do
			{
				...
				case CookieToken.NameValuePair:
					...
					case CookieToken.Expires:
						...
						{
							flag4 = true;
							if (DateTime.TryParse(CheckQuoted(_tokenizer.Value), CultureInfo.InvariantCulture, DateTimeStyles.AllowWhiteSpaces, out var result4))
							{
