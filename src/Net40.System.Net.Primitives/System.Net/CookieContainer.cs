...
public class CookieContainer
{
    ...
    private string m_fqdnMyDomain = s_fqdnMyDomain;

    public int Capacity
    {
        get { return m_maxCookies; }
        set
        {
            if (value <= 0 || (value < m_maxCookiesPerDomain && m_maxCookiesPerDomain != int.MaxValue))
            {
                throw new ArgumentOutOfRangeException("value",
                    System.SR.Format(System.SR.net_cookie_capacity_range, "Capacity", 0, m_maxCookiesPerDomain));
            }
    ...
    {
        get { return m_maxCookiesPerDomain; }
        set
        {
        ...
        : this(capacity)
    {
        if (perDomainCapacity != int.MaxValue && (perDomainCapacity <= 0 || perDomainCapacity > capacity))
        {
            throw new ArgumentOutOfRangeException("perDomainCapacity",
                System.SR.Format(System.SR.net_cookie_capacity_range, "PerDomainCapacity", 0, capacity));
        }
    ...
    public void Add(Cookie cookie)
    {
        ...
        {
            throw new ArgumentException(System.SR.Format(System.SR.net_emptystringcall, "cookie.Domain"),
                "cookie.Domain");
        }
        ...
        cookie2.VerifySetDefaults(cookie2.Variant, result, IsLocalDomain(result.Host), m_fqdnMyDomain, setDefault: true,
            shouldThrow: true);
    ...
    internal void Add(Cookie cookie, bool throwOnError)
    {
        ...
        try
        {
            ...
            {
                if ((cookiesCount >= m_maxCookiesPerDomain && !AgeCookies(cookie.DomainKey)) ||
                    (m_count >= m_maxCookies && !AgeCookies(null)))
                {
                    return;
                }
    ...
    internal bool IsLocalDomain(string host)
    {
        ...
        switch (host)
        {
            ...
            default:
            {
                if (string.Compare(m_fqdnMyDomain, 0, host, num, m_fqdnMyDomain.Length,
    ...
    public void Add(Uri uri, Cookie cookie)
    {
        ...
        Cookie cookie2 = cookie.Clone();
        cookie2.VerifySetDefaults(cookie2.Variant, uri, IsLocalDomain(uri.Host), m_fqdnMyDomain, setDefault: true,
            shouldThrow: true);
        Add(cookie2, throwOnError: true);
    }
    ...
    public void Add(Uri uri, CookieCollection cookies)
    {
        ...
        foreach (Cookie cookie3 in cookies)
        {
            ...
            cookie2.VerifySetDefaults(cookie2.Variant, uri, isLocalDomain, m_fqdnMyDomain, setDefault: true,
                shouldThrow: true);
            Add(cookie2, throwOnError: true);
        }
    }
    ...
    internal CookieCollection CookieCutter(Uri uri, string headerName, string setCookieHeader, bool isThrow)
    {
        ...
        try
        {
            ...
            while (true)
            {
                ...
                else if (cookie.VerifySetDefaults(variant, uri, isLocalDomain, m_fqdnMyDomain, setDefault: true,
                             isThrow))
                {
    ...
    private void BuildCookieCollectionFromDomainMatches(Uri uri, bool isSecure, int port, ref CookieCollection cookies,
        List<string> domainAttribute, bool matchOnlyPlainCookie)
    {
        for (int i = 0; i < domainAttribute.Count; i++)
        {
            ...
            lock (pathList.SyncRoot)
            {
                ...
                while (enumerator.MoveNext())
                {
                    ...
                    if (uri.AbsolutePath.StartsWith(CookieParser.CheckQuoted(value)))
                    {
                        CookieCollection cookieCollection = (CookieCollection)enumerator.Value;
    ...
    private void MergeUpdateCollections(ref CookieCollection destination, CookieCollection source, int port,
        bool isSecure, bool isPlainOnly)
    {
        lock (source)
        {
